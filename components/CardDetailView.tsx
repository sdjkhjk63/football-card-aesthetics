  ...
  return (
    <main className="detail-page">
      ...
      <div className="detail-layout">
        <div className="detail-media"><div className={design.layout === "landscape" ? "detail-visual landscape" : "detail-visual"}>{imageIsVerified ? <Image src={`/${design.image.path}`} alt={localize(design.image.alt, locale)} width={900} height={1200} priority /> : <div className="card-image-unverified" role="status"><span>{unverifiedLabel}</span><small>{design.officialName}</small></div>}{design.serial && <span className="serial-badge large">{design.serial}</span>}</div><CuratorNote note={design.curatorNote} locale={locale} cardSlug={design.slug} repository={authorNoteRepository} editable={authorMode} /></div>
        <div className="detail-copy"><span className="eyebrow">{design.group.toUpperCase()} · {design.section.replaceAll("-", " ").toUpperCase()}</span><h1>{localize(design.name, locale)}</h1><p className="official-name">{design.officialName}</p>{design.parallels?.length ? <div className="parallel-panel"><span>PARALLELS</span>{design.parallels.map((parallel) => <small key={`${parallel.name}-${parallel.serial ?? "base"}`}>{parallel.name}{parallel.serial ? ` ${parallel.serial}` : ""}</small>)}</div> : null}<RatingForm cardSlug={design.slug} repository={repository} locale={locale} communityRepository={communityRepository} deviceId={deviceId} /><FeedbackForm cardSlug={design.slug} locale={locale} repository={feedbackRepository} /></div>
      </div>
    </main>
  );
}
